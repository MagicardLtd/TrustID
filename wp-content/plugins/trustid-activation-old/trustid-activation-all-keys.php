...
//---------------------------------------------------------------------------------
	...
	{ //This must be a new page, so check if there's any session data to carry across from previous page
		...
		else if (!empty($_SESSION['skSearchField']))
	...
	if (!empty($writeSearchField) && !empty($searchTerm))
	{
        ...
        if (current_user_can("view_PII"))
		{
			$columns=array(
                ...
                'copyId'=>__('Copy ID'),
				'activationCount'=>__('Activation Count'),
                'active'=>__('Status'),
				'edit'=>__('Edit')
            );
			$restricted_info = "activationCount,";
		}
		else
		{
			$columns = array(
...
//---------------------------------------------------------------------------------
	...
	$searchUrl = $plugins_url."admin.php?page=tid_activation&registrationkey=";
