...
function trustid_activate_shortcode($atts = array(), $content=null, $code="") {
    ...
    if (!empty($_POST['generate']))
    {
        ...
        if (isset($tidActivationResult['Status']) && $tidActivationResult['Status'] == 0)
        {
            ...
            $copy = get_post_custom_values("Activation-Success");
            $response = $copy[0].'<br /><br />';
            $response .= '<input class="code" type="text" value="'.$tidActivationResult['ResponseCode'].'" id="myInput">';
            $response .= '<button class="button-primary codeBtn" onclick="myFunction()">Copy Activation Code</button>';
            $response .= '<script>
              function myFunction() {
                var copyText = document.getElementById("myInput");
                copyText.select();
                document.execCommand("Copy");
                alert("Code copied successfully: " + copyText.value);
              }
            </script>';
            return $response;
        }
